...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
        ...
        self.base_layer = nn.Sequential(
            ...
            nn.ReLU(inplace=True))  # (bs, 3, h, w) -> (bs, 16, h, w)
        self.level0 = self._make_conv_level(
            channels[0], channels[0], levels[0])  # (bs, 16, h, w) -> (bs, 16, h, w)
        self.level1 = self._make_conv_level(
            channels[0], channels[1], levels[1], stride=2)  # (bs, 16, h, w) -> (bs, 32, h/2, w/2)
