...
class STrack(BaseTrack):
    ...
    def __init__(self, cls, tlwh, score, temp_feat, buffer_size=30):
...
class mySTrack(STrack):

    def __init__(self, cls, tlwh, score, buffer_size=30):
        # wait activate
        self.cls = cls  # category
        self._tlwh = np.asarray(tlwh, dtype=np.float)
        self.kalman_filter = None
...
class JDETracker(object):
    ...
    def post_process(self, dets, meta):
        """
        Returns:
            dict, key: cls_id (range: 1 ~ num of cls), value: np.ndarray, shape: (num of objs, 5)
        """
    ...
    def update(self, im_blob, img0):
        ...
        with torch.no_grad():
            ...
            id_feature = output['id']  # shape: (bs, dim, h, w)
...
class myTracker(JDETracker):
    ...
    def update(self, im_blob, img0):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [mySTrack(cls, STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], 30) for
                          (cls, tlbrs) in zip(dets[:, 5], dets[:, :5])]
        else:
            detections = []
